...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white text-app-body-text">
      <header className="bg-app-header text-white p-4 shadow-md">
        <div className="container mx-auto flex flex-wrap justify-between items-center">
          <h1 className="text-2xl font-bold text-app-accent">Life Cycle Design Toolkit</h1>
          <nav className="mt-2 md:mt-0">
            <ul className="flex flex-wrap gap-2 md:gap-4">
              {navItems.map((item) => (
                <li key={item.path}>
                  <Button asChild variant="ghost" className={cn(
                    "text-white hover:bg-app-accent hover:text-app-header",
