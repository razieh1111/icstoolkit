...
const StrategyInsightBox: React.FC<StrategyInsightBoxProps> = ({
  ...
  return (
    <Draggable handle=".handle"> {/* Wrap with Draggable and define a handle */}
      <div className={cn(
        "bg-white p-3 rounded-lg shadow-md border border-gray-200 flex flex-col",
        "w-72 h-48", // Changed width from w-64 to w-72
        className
      )} style={style}>
        <div className="flex items-center mb-2 handle cursor-grab"> {/* Add handle class and cursor style */}
          <span className={cn(
